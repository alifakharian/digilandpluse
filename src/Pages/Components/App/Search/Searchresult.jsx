...
export default function Searchresult() {
  // grid grid-cols-6 gap-[15px] py-[30px]  p-2  justify-center smd:grid-cols-1 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 dark:bg-slate-900 dark:text-white
      ...
      >
        {filteredItems.length > 0 ? (
          filteredItems.map((elem) => (
            <div className="" key={elem.i}>
              <Link to={`/Products/${elem.i}`}>
                <div
                  className=" dark:border-blue-900 rounded-lg border-2"
                  dir="rtl"
                >
                  <img src={elem.image} className="w-[150px] rounded-full dark:mt-2 dark:border-[2.5px] dark:border-blue-600 mx-auto" />
